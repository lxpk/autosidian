...
- **Auto–Iconize (v1.0+):** Longest **keyword** match on the **folder name** → set configurable front matter (default `icon`) on the **folder note**; built-in + JSON-imported rules **plus per-emoji custom keywords** edited inline in the bundled-emoji table. Optional **Match most similar emoji** uses [emojilib](https://github.com/muan/emojilib) (biased by your custom keywords). New/rename folder + per-minute retro. [UI.md](UI.md#autoiconize) · [iconize](src/iconize/).
...
- **Auto–Cover (v1.1+):** Alternative to Auto–Pixel Banner that uses Obsidian's **built-in `cover` property** (no community plugin). Performs a real web image search per note (Wikipedia → Openverse, optionally Pexels with API key) and writes the resulting image URL into front matter, where Bases card view and Publish previews pick it up. New note (optional), retro, picker command. [UI.md](UI.md#autocover) · [autoCover](src/autoCover/).
- **Hierarchical folder styling (planned):** Apply visible per-depth styling to hierarchical folders so each level of the tree is recognizable across **graph view**, **file explorer**, and (where applicable) folder notes. Examples: bigger graph nodes for root/top-level folders, scaled font/icon size per depth, level-based color or weight. Configuration: per-depth style table (size, color, font weight, icon scale), max depth, and toggles per surface (graph / explorer / banner). Implementation will likely combine Obsidian's [graph view CSS](https://help.obsidian.md/Plugins/Graph+view) overrides, dynamic CSS variables in [styles.css](styles.css), and metadata writes (e.g. depth tag in front matter) compatible with [Iconize](src/iconize/) and [Auto–Cover](src/autoCover/). [UI.md](UI.md#hierarchical-folder-styling-planned) · [PLANS.md](PLANS.md#phase-7--hierarchical-folder-styling-planned).
